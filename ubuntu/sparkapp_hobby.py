...
file_db_mapping = [
    ("/data/hobby.csv", "mallangdb.hobby")
]

for file_path, target_table in file_db_mapping:
    # CSV 파일을 읽어옵니다.
    hobby = spark.read.format("csv").option("header", "true").option("inferSchema","true").load(file_path)
    ...
    hobby.write.jdbc(url=jdbc_url, table=target_table, mode="overwrite", properties=properties)
