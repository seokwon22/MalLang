...
def diary_check(request):
    ...
    if request.method == 'POST':
        ...
        # 그림
        openai.api_key = "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"
...
def detail(request, id):
    ...
    return render(request, "MalLang/detail.html", {"dto": board, "image_path": image_path, 'context': context})
...
def search_engine(request):
    es = Elasticsearch(
    hosts=['http://15.152.136.200:9200'],
...
class SearchView(APIView):
    ...
    def get(self, request):
        es = Elasticsearch(
            hosts=[{'host':'15.152.136.200', 'port':9200, 'scheme':"http"}],
        ...
        # print(docs)
        # data_list = docs['hits']
        titles = [hit['_source']['diarycontent'] for hit in docs['hits']['hits']]
        # print(titles)

        # return Response(data_list)
        # return render(request, 'search_results.html', {'search_results': data_list})
        return render(request, 'MalLang/search_results.html', {'search_results': titles})
